...
import {
  User, Mail, Phone, Lock, Save, X, Eye, EyeOff,
  BookOpen, Camera, AlertCircle, Loader2,
} from "lucide-react";
...
export default function ParentProfilePage() {
    ...
    async function load() {
      ...
      try {
        ...
        if (parentProfileId) {
          ...
          if (found) { parentData = found; setParentProfileId(found.id); }
        }
  ...
  const studentName     = student ? `${student.user.firstName} ${student.user.lastName}`.trim() : null;
  const studentInitials = studentName ? studentName.split(" ").map(n => n[0]).join("").slice(0, 2).toUpperCase() : "—";
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 24, flexWrap: "wrap" }}>
            <div style={{ position: "relative", flexShrink: 0 }}>
              <div style={{
                ...
                {avatarUrl
                  ? <img src={avatarUrl} alt={displayName} key={avatarUrl} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                  : initials
                }
              </div>
              <button
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {!editing ? (
                ...
                <button onClick={() => setEditing(false)} className="btn btn-ghost btn-sm"><X size={13} /> Batal</button>
              )}
            </div>
          </div>
        </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "14px 16px", background: "var(--primary-light)", borderRadius: 10, border: "1px solid rgba(2,126,116,.15)" }}>
              <div style={{ width: 48, height: 48, background: "var(--primary)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 16 }}>
                {student.user.avatarUrl
                  ? <img src={student.user.avatarUrl} alt={studentName ?? ""} style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "50%" }} />
                  : studentInitials
                }
              </div>
              <div style={{ flex: 1 }}>
